define(["dojo/_base/declare", "esri/layers/tiled"], function (declare) {
    return declare(esri.layers.TiledMapServiceLayer, {
        ...
        getTileUrl: function (level, row, col) {
            // 上海2000坐标系 → EPSG:3857 瓦片坐标转换
            // 默认偏移 (zoom=10): dcol=345, drow=-93
            // 在浏览器 Console 中实时调优:
            //   window._TILE_DCOL = 345;   // 调整 X 方向 (正=向东)
            //   window._TILE_DROW = -93;   // 调整 Y 方向 (负=向北)
            ...
            var layerUrl = "http://10.91.7.102:8093/tdt_tile/img_w/" + level + "/" + stdRow + "/" + stdCol + ".png?jcmp-token=d01b7f932ff44c04bd0b026e9daaae97&_v=" + (window._TILE_VER || 0);
            if (this._logCount < 10) {
                this._logCount++;
                console.log("LocalImgLayer[" + this._logCount + "]: z=" + level + " API(" + row + "," + col + ") → fix(" + stdRow + "," + stdCol + ") dcol=" + dcol + " drow=" + drow);
            }
            return layerUrl;
        }
    });
