...
function YLload() {
  ...
  if (strJson.length > 0) {
    for (var i = 0; i < strJson.length; i++) {
      ...
      if (f > 0) {
        // if (RainfallStr == "") {
        //    RainfallStr += item.lgtd84 + "," + item.lttd84 + "," + f;
        // } else {
        //    RainfallStr += "|" + item.lgtd84 + "," + item.lttd84 + "," + f;
        // }
        RainfallStr.push({ longitude: item.lgtd, latitude: item.lttd, value: f });
      }
    }
...
function MapRainfall() {
  ...
  apidzxm.rainfallIsosurfaces(strParam).then((res) => {
    MapRainfallData(res);
  });
}

async function MapRainfallData(res) {
  var isolines = res.data;
  var isolinesNew = [];
  // 使用 for...of 循环，配合 await，实现顺序执行
  // res.data.forEach(function (surface, index) {
  //   var color = colors[index] || colors[colors.length - 1] || "#000";
  //   surface.polygons.forEach(function (coords) {
  //     // 转换坐标顺序：[lon, lat] -> [lat, lon]
  //     // var latLngs = coords.map(function (coord) {
  //     //   return [coord[1], coord[0]];
  //     // });
      
  //     // 1. 拼接坐标
  //     const geometries = coords
  ...
  //     // 3. 发起请求并等待结果
  //     // 这里的 await 会阻塞循环，直到请求完成
  //     const resXY = await arcgisonIndex.GeometryServerProject(strWhere);
  //   });
  // });
  var index=0;
  for (const u of isolines) {
    ...
    for (const coords of polygons) {
      ...
      try {
        ...
        // const strWhere = {
        //   geometries: geometries,
        //   // geometryType: "esriGeometryPolyline", // 根据接口文档添加必要参数
        // };
        ...
        // // 4. 处理返回数据
        // // console.error("接口返回:", resXY);
        // const coordinatesNew = resXY.geometries.map((g) => [g.x, g.y]);

        // // 5. 更新原始数据
        // // u.coordinates = coordinatesNew;
        // // results.push(u);


        
      // console.error("coordinatesNew:", coordinatesNew);
      PointMark.MapRainfallSing(coords,color);
    } catch (err) {
        console.error("请求失败:", err);
    }

      // console.error("所有请求完成", results);
      // isolinesNew.push(results);
    }
    index++;
  }
