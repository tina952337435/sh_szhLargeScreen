...
function loadZhan(){
	...
	api.loadModeZhanDianJY(strParam)
		.then(res => {
...
function onLoadData() {
	...
	api
		.loadModeZhanDianDataBindJY(strParam)
		.then(res => {
...
function ylHtml(data) {
	...
	zhanData.map(e=>{
		...
		for (var num = 0; num < data.length; num++) {
			if(data[num][e.id] != undefined){
				drp += parseFloat(data[num][e.id]);
			}
		}
		result.push({name:e.title,drp:drp.toFixed(1)});
	})
