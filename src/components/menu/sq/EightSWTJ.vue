<template>
  <div class="m-box">
    <div class="title display_flex justify-content_flex-start align-items_center leftTop-radius layout_title px-2">
      <div class="d1"></div>
      <div class="d2"></div>
      <p class="base-p" id="title2" @click="fangda()">8时水雨情统计</p>
      <span class="spanTitle"></span>
    </div>
    <div class="txt">
      <div class="m-list4 box-siz" id="tableSY" style="height: 90%; margin: 2% auto">
        <div class="flex_1 width95 maiweid display_flex mhpad1 flex-wrap justify-content_flex-justify"
          style="width: 94%; margin: 0 auto; height: 60%">
          <div class="widget-inline-box text-center fl liuyupjbg">
            <p>&lt;25<br />(小~中雨)</p>
            <h3 class="def1ff" ref="Color25" style="height: 50px; line-height: 26px;color:var(--titled1);" @click="YQtable('count25')">
              {{ count25 }}
            </h3>
          </div>
          <div class="widget-inline-box text-center fl liuyupjbg">
            <p>25~50<br />(大雨)</p>
            <h3 class="def1ff" ref="Color50" style="height: 50px; line-height: 26px;color:var(--titled1);" @click="YQtable('count50')">
              {{ count50 }}
            </h3>
          </div>
          <div class="widget-inline-box text-center fl liuyupjbg">
            <p>50~100<br />(暴雨)</p>
            <h3 class="def1ff" ref="Color100" style="height: 50px; line-height: 26px;color:var(--titled1);" @click="YQtable('count100')">
              {{ count100 }}
            </h3>
          </div>
          <div class="widget-inline-box text-center fl liuyupjbg">
            <p>100~250<br />(大暴雨)</p>
            <h3 class="def1ff" ref="Color250" style="height: 50px; line-height: 26px;color:var(--titled1);" @click="YQtable('count250')">
              {{ count250 }}
            </h3>
          </div>
          <div class="widget-inline-box text-center fl liuyupjbg">
            <p>&gt;250<br />(特大暴雨)</p>
            <h3 class="def1ff" ref="ColorBig" style="height: 50px; line-height: 26px;color:var(--titled1);" @click="YQtable('countBig')">
              {{ countBig }}
            </h3>
          </div>
        </div>
        <div class="flex_1 maiweid display_flex mhpad1 flex-wrap justify-content_flex-justify"
          ...
          <div class="ztitem ztitem1ps display_flex flex-direction_column">
            ...
            <div
              ...
              <div class="yjtxtcon">
                <span class="yjbigtxt def1ff" ref="WRZColor" @click="SQtable('wrzCount')">{{ wrzCount }}</span>
              </div>
            </div>
          </div>
          <div class="ztitem ztitem1ps display_flex flex-direction_column">
            ...
            <div
              class="flex_1 maiweid display_flex flex-direction_column justify-content_flex-center align-items_center hand">
              <div class="yjtxtcon">
                <span class="yjbigtxt def1ff" ref="nullColor" @click="SQtable('nullCount')">{{ nullCount }}</span>
              </div>
            </div>
          </div>
          <div class="ztitem ztitem1ps display_flex flex-direction_column">
            ...
            <div
              class="flex_1 maiweid display_flex flex-direction_column justify-content_flex-center align-items_center hand">
              <div class="yjtxtcon">
                <span class="yjbigtxt def1ff" ref="ZCColor" @click="SQtable('zcCount')">{{
                  zcCount
                }}</span>
              </div>
            </div>
  ...
  <MyDialog :showDialog="showDialogSW" @close="showDialogSW = false" :title="titleNameSW" :typeValue="typeValueSW"
    style="width: 70%; height: 700px" v-else="shuju == 'SQ'">
    <SQTJ />
  </MyDialog>
...
import EightSWTJ from "@/components/menu/sq/EightSWTJ.vue";
...
import { onMounted, ref, shallowRef, defineAsyncComponent, nextTick, provide } from "vue"; // 获取当前主题
...
function Weacontent() {
  ...
  //   .then((res) => {
    ...
    "expand": null
      ...
      var dataResult = groupBy(res.result, "stcd");
      for (var num = 0; num < dataResult.length; num++) {
        var item = dataResult[num][0];
        var wrz = "—";
        if (SetNull(item.wrz) != "") {
          wrz = Number(item.wrz).toFixed(2);
        }
        var grz = "—";
        if (SetNull(item.grz) != "") {
          grz = Number(item.grz).toFixed(2);
        }
        var upz = "—";
        if (SetNull(item.upz) != "") {
          upz = Number(item.upz).toFixed(2);
        }

        if (SetNull(item["tm"]) != "") {
          if (grz != "—" && grz > 0 && Number(upz) >= Number(grz)) {
            grzCount.value++;
          } else if (wrz != "—" && wrz > 0 && Number(upz) >= Number(wrz)) {
            wrzCount.value++;
          } else {
            zcCount.value++;
          }
        ...
        if (zcCount.value > 0) {
          if (_theme == "BlueTheme") {
            // ZCColor.value.style.color = "#04F1B2";
            // ZCColor.value.style.color = "rgb(64,252,0)";
          } else if (_theme == "default") {
            ZCColor.value.style.color = "#16FF8D";
          }
        }
        if (wrzCount.value > 0) {
          if (_theme == "BlueTheme") {
            WRZColor.value.style.color = "#F37804";
          } else if (_theme == "default") {
            WRZColor.value.style.color = "#FF9E43";
          }
        }
      }
    // })
