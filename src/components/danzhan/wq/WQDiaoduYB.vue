<template>
  <div class="tableDiv tableDivWQ" style="height:630px;width:100%;display:none;">
    <el-table class="m-table" style="height:calc(100% - 30px);width:100%;--el-table-border-color:none;" :data="tableData"
      ...
      <el-table-column label="圩外预报水位" header-align="center" align="center">
        <template #default="scope">
          <span v-if="SetNull(scope.row.stnmw) != ''">{{ scope.row.stnmw }}{{ scope.row.upz }}米</span>
        </template>
      </el-table-column>
      <el-table-column label="圩内预报水位" header-align="center" align="center">
        <template #default="scope">
          <span v-if="SetNull(scope.row.stnmn) != ''">{{ scope.row.stnmn }}{{ scope.row.dwz }}米</span>
        </template>
      </el-table-column>
  ...
  <div class="tableDiv tableDivQT" style="height:630px;width:100%;display:none;">
    <el-table class="m-table" style="height:calc(100% - 30px);width:100%;--el-table-border-color:none;" :data="tableData"
      ...
      <el-table-column label="预报水位" header-align="center" align="center">
        <template #default="scope">
          <span v-if="SetNull(scope.row.stnmw) != ''">{{ scope.row.stnmw }}{{ scope.row.upz }}米</span>
        </template>
      </el-table-column>
      <!-- <el-table-column label="圩内预报水位" header-align="center" align="center">
        <template #default="scope">
          <span>{{ scope.row.stnmn }}{{ scope.row.dwz }}米</span>
        </template>
      </el-table-column> -->
...
function Weacontent() {
  ...
  var strParam = { "pathname": props.wqid, "datasource": datasource, "etime": props.etime };//"strExp":"true"
  if (datasource == "YB") {
    strParam["pid"] = pid;
  }
  api.wqbaseddgzFindResult(strParam).then(res => {
    var resultData = groupBy(sortObjectArray(res.result, ["flag"], "desc"), "tm");
    // console.error("resultData", resultData)
    var resultJson = [];
    if (resultData.length > 0) {
      for (let num = 0; num < resultData.length; num++) {
        resultData[num][0]["tm"] = dayjs(resultData[num][0]["tm"]).format("YYYY-MM-DD HH:mm");
        resultJson.push(resultData[num][0])
      }
    }
    tableData.value = resultJson;
    // tableData.value = groupBy(sortObjectArray(res.result, ["flag"], "desc"), "tm");
