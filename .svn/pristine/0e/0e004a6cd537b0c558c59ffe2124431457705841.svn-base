    ...
    <div class="m-box">
        <div class="title display_flex justify-content_flex-start align-items_center leftTop-radius layout_title px-2">
            <div style="line-height:30px;">
                <div class="d1"></div>
                <div class="d2"></div>
                <p class="base-p1" id="title2">测站级别</p>
                <div class="div-swiper" style="width: calc(100% - 130px);position: absolute;left: 130px;top: 5px;">
                    <div class="swiper-slide" style="width: 50%;"
                        :class="StcdType == 'nei' && 'swiper-slide swiper-slide-thumb-active'"
                        @click="getStcdType('nei')">
                        流域内</div>
                    <div class="swiper-slide" style="width: 50%;"
                        :class="StcdType == 'wai' && 'swiper-slide swiper-slide-thumb-active'"
                        @click="getStcdType('wai')">
                        流域片
                    </div>
                </div>
            </div>
...
function Weacontent() {
    ...
    var pieArrYSL = [{ "name": "国家重要水文测站", "value": 8 }, { "name": "一般水文测站", "value": 56 }];
    var pieArrPSL = [{ "name": "专用水文测站", "value": 803 }];
    // if (strJson.length > 0) {
    ...
    const _OptionRu = ChartJs.echartSLpei(dateidRu.value, pieArrYSL, LineColor, _theme, "", [], "个", false, 0);
    ...
    trendsTooltipRu.value = setInterval(function () {
        ...
        // 当前高亮图形
        currentIndexRu = (currentIndexRu + 1) % dataLenRu;
        myChartRu.dispatchAction({
            type: "highlight",
            seriesIndex: 0,
            dataIndex: currentIndexRu,
    ...
    const _OptionChu = ChartJs.echartSLpei(dateidChu.value, pieArrPSL, LineColor, _theme, "", [], "个", false, 0);
