...
import { ref} from "vue";
  var exportObj =ref({});
  var clearAnimationFrame=null;
  function  initAll() {
    var height=$(window).height();
    $('#app').height(height*window.devicePixelRatio);
    ...
    function SiriWave(opt) {
      ...
      if($(window).height() < 700)this.height = 120;
      this.MAX = (this.height / 2) - 4;
      this.canvas = $("#wave")[0];
      this.canvas.width = this.width;
      this.canvas.height = this.height>150?150:this.height ;
      if($(window).height() < 700)this.canvas.height = 120;
      this.canvas.style.width = '100%';//(this.width / devicePixelRatio) + "px";
      this.canvas.style.height = '180px'//(this.height / devicePixelRatio) + "px";
    ...
    SiriWave.prototype = {
      ...
      _draw: function() {
        ...
        this._clear();
        this._drawLine(1,"rgba(41,147,255,0.1)", 1, 0.2, 1);
        this._drawLine(0.5, "rgba(41,147,255,0.2)", 1, 0.35, 2);
