      ...
      <div class="Main">
        ...
        <div style="float:left;width:calc(100% - 10px);height:580px;background-color:transparent;margin-left:10px;">
          <div class="layout_title px-2">
            <div data-v-62547c07 class="d1"></div>
            <div class="d2"></div>
            <p class="base-p" id="title1">水情预报</p>
...
onMounted(() => { 
  mini.parse();
  mini.get("STIME").setValue(dayjs(stime.value).add(-2, "hour").format("YYYY-MM-DD HH:00"));
  // alert(props.tableData.length);
  ybdrplist.value = props.tableData;

  drpSearch(ybdrplist.value);
  Weacontent();
});
// const props = defineProps({
//   stcd: {
...
function drpSearch(data) {
  console.error("datadrpSearch", data)
  var result = [];
  var sumdrp = 0;
...
function Weacontent() {
  ...
  if (pathname.value == "预报") {
    ...
    console.error("ybdrplist", ybdrplist.value)
    strParam["ybdrplist"] = JSON.stringify(ybdrplist.value);
  }
  else {
    strParam["xiaoyan"] = "FANYAN";
    if (tabWQnameType.value == "预报降雨") {
      strParam["ybdrplist"] = JSON.stringify(tabWQname.value);
    }
    console.error("====",tabWQnameType.value,tabWQname.value)
  }
  ...
  api
    ...
    .then(res => {
      ...
      if (strJson.length > 0) {
        ...
        console.error("waterListDesc", waterListDesc,strJson)
        if(SetNull(strJson[0]["avgupz"])!=""){
          $("#WQUPZ").html(Number(strJson[0]["avgupz"]).toFixed(2));
        }
         if(SetNull(strJson[0]["wqwrz"])!=""){
            $("#WQQPSW").html(Number(strJson[0]["wqwrz"]).toFixed(2));//控制水位
         }

       if(SetNull(strJson[0]["wqgrz"])!=""){
            $("#WQAPSW").html(Number(strJson[0]["wqgrz"]).toFixed(2));//警戒水位
         }
        if(SetNull(strJson[0]["pumcap"])!=""){
           $("#PUMCAP").html(Number(strJson[0]["pumcap"]).toFixed(1));
         }
         if(SetNull(strJson[0]["plsjpum"])!=""){
           $("#PLSJPUM").html(Number(strJson[0]["plsjpum"]).toFixed(1));
         }
          if(SetNull(strJson[0]["wq_tlow"])!=""){
            $("#WQ_TLOW").html(Number(strJson[0]["wq_tlow"]).toFixed(2));
         }
        ...
        if (pathname.value == "校验") {
          ...
          ((Number($("#YBDRP").html()) / 1000) *
            Number($("#WQWWMJ").html())*1000000 *
            (Number($("#wqplength").val())/100)) /10000;
            console.error("sumDRPSLNote",$("#YBDRP").html(),$("#WQWWMJ").html(),$("#wqplength").val(),sumDRPSLNote);
        $("#YBDRPSL").html(sumDRPSLNote.toFixed(2));
...
function OnBoot(type) {
  ...
  console.error("type",type,props,tableData,tableDataLL)
  propsdrplist.value=props;
  console.error("ybdrplist",propsdrplist.value)
