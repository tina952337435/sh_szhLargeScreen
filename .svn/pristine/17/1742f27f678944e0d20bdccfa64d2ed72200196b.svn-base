<template>
  <div class="m-box m-box-1" style="position: relative">
    <div class="title display_flex justify-content_flex-start align-items_center leftTop-radius layout_title px-2">
      <div class="d1"></div>
      <div class="d2"></div>
      <p class="base-p" id="title2" @click="fangda()">积水点巡查</p>
...
function handleclick(evt) {
  ...
  if (_lgtd == "" || _lttd == "") {
    ...
    var nowTM = new Date();
    evt["stime"] = dayjs(dayjs(nowTM).format("YYYY-MM-DD 00:00:00")).add(-1, "month").format("YYYY-MM-DD HH:mm:ss");
    evt["etime"] = dayjs(nowTM).format("YYYY-MM-DD HH:mm:ss");
    evt["showtype"] = "addWaterJSDMark";
  }
}
onMounted(() => {
  var now = new Date();
