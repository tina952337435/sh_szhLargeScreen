<template>
  <div class="m-box m-box-2" style="position: relative">
    <div class="title display_flex justify-content_flex-start align-items_center leftTop-radius layout_title px-2">
      <div class="d1"></div>
      <div class="d2"></div>
      <p class="base-p" id="title2" @click="fangda()">雨中巡查</p>
    ...
    <div class="txt">
      <el-table :data="tableData" @row-click="handleclick"
        ...
        <el-table-column label="名称" prop="address" style="width:40%" align="center" :show-overflow-tooltip="true">
        </el-table-column>
        <el-table-column label="类型" prop="type" style="width:20%" align="center">
        </el-table-column>
        <el-table-column label="状态" prop="state" style="width:20%" align="center" :show-overflow-tooltip="true">
        </el-table-column>
      </el-table>
    </div>
    <div class="bot"></div>
...
function Weacontent() {
  ...
  api.DangerSelectList(strParam)
    .then(res => {
      var listJson=groupBy(res.result,'eyldbm');
      var strJson = [];
      if(listJson.length>0){
        for(var i=0;i<listJson.length;i++){
          var listParam=sortObjectArray(listJson[i],['tm'],"desc");
          if(listParam.length>0){
            strJson.push(listParam[0])
          }
        }
      } 
      ...
      if (strJson.length > 0) {
        for (var num = 0; num < strJson.length; num++) {
          ...
          if (strJson[num].state == "1") {
            // ["待交办", "处理中", "待审核", "已完成"]
            item["state"] = "待交办";
          } else if (strJson[num].state == "2") {
            item["state"] = "处理中";
          } else if (strJson[num].state == "3") {
            item["state"] = "待审核";
          } else if (strJson[num].state == "4") {
            item["state"] = "已完成";
          }else if (strJson[num].state == "0") {
            item["state"] = "待交办";
          }
          result.push(item)
        }
