<template>
  <div class="m-box">
    <div class="title display_flex justify-content_flex-start align-items_center leftTop-radius layout_title px-2">
      <div class="d1"></div>
      <div class="d2"></div>
      <p class="base-p" id="title2" @click="fangda()">昨日累计雨量排名前5</p>
      <span class="spanTitle"></span>

      <label style="margin-left:175px;"  @click="more()">
  ...
  <ComZujian :showDialog="showDialog" @close="showDialog = false" :title="titleName" :typeValue="typeValue"
    ...
    <EchartYQTop :typenameRadio="props.typenameRadio" />
  </ComZujian>
</template>
<script setup>
...
function fangda() {
  ...
  dateid.value = "daytop1";
}
function more() {
    const ChildVue = defineAsyncComponent(() =>
            import("@/components/danzhan/yq/TableYQTop.vue")
        );
        // console.error("单站过程", item);
        const props = {};
        props["data"] = resultData.value;
        //ChildVue为弹窗中嵌入的slot
        Dialog.open({ title: "昨日累计雨量排名", widh: 1500, heig: 700 }, h(ChildVue, props)).then(() => { console.log('弹窗关闭了') })
}
onMounted(() => {
  var now = new Date();
