...
function Weacontent() {
    hedaoTitle.value=props.ZhanDanSW.length>0?props.ZhanDanSW[0].name:"";
    ...
    if (SOLUTIONIDList.length > 0) {
        ...
        $.data(myData, "DD_NAMES", SOLUTIONIDNAME);
        $.data(myData, "DD_IDS", SOLUTIONID);
        stcd.value =props.ZhanDanSW.length>0?props.ZhanDanSW[0].stcd:"";
        GetFangAnData(SOLUTIONID, stcd.value);
    }

    $("#SWDBZLIST li").click(function (res) {
    //   console.error(res.currentTarget, res.currentTarget.name)
      $("#SWDBZLIST li").removeClass("liSelected");
      $(this).addClass("liSelected");
      $("#SWDBZLIST").css("display", "none");
      var Temp = props.ZhanDanSW.filter(function (e) {
...
function gridLoad() {
    ...
    const _Option = ChartJs.chartSW("", dataResult, strNote, LineColor, "水位", "Mouth", _theme, 55, 14);
